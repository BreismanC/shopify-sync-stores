...
export function WhatsNewCard({
  ...
  return (
    <Card className="min-h-68 rounded-lg border border-outline-variant bg-surface-container-lowest p-6 shadow-sm">
      <div className="grid h-full grid-cols-1 items-center gap-8 md:grid-cols-[minmax(240px,0.9fr)_minmax(320px,1.1fr)]">
        <div>
          <h3 className="text-2xl font-semibold leading-tight tracking-tight text-on-background">
            {featureTitle}
          </h3>
          <p className="mt-6 max-w-80 text-base leading-6 text-on-surface-variant">
            {description}
          </p>
          {bullets.map((bullet) => (
            <p
              key={bullet}
              className="mt-4 max-w-80 text-base leading-6 text-on-surface-variant"
            >
              {bullet}
            </p>
          ))}
          <Button
            asChild
            className="mt-6 h-10 rounded-md bg-primary px-5 text-sm font-semibold text-primary-foreground hover:bg-primary/90"
          >
            <Link href={learnMoreHref}>Conocé más</Link>
          </Button>
        </div>
